...
The sales-leadership plugin is the leader-shaped view over the six schema
slots. It supports four modes:
...
- `WIN_LOSS`: pattern refresh over closed-won and closed-lost cohorts, grouped
  by category labels from the controlled vocabularies in
...
optimizer. It answers "which opportunities pass each set, and where do the
deltas fall?", not "what is the revenue-maximizing allocation subject to these
constraints?" Real constrained optimization is a stretch goal, and the plugin
says so if the operator asks for optimization.
