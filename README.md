...
through skills: the base stays motion-agnostic, while each skill binds the
schema slots, agent roster, persona plugin defaults, cookbook set, connector
bindings, and motion-specific theory constants.
...
- **`ae`**: account-executive-shaped views.
- **`sales-leadership`**: pipeline-level views and board-vs-plan deltas.
- **`revops`**: schema health, source quality, coverage gaps.
- **`competitive-intel`**: competitor signals from trigger and outcome data.
...
- **Morning dossier**: daily per-account briefing assembled across the reference
  skill slots.
...
Connectors default to read-only. Write operations require explicit operator
opt-in. See `connectors/`.
